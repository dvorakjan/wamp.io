...
Server.prototype.onConnection = function(client) {
  ...
  // Reverse RPC functions
  client.calls = {};
  client.call = function(procedure, params, callback) {
    var callId = Math.round(Math.random()*100000)+'';

    msg = [protocol.TYPE_ID_REVERSE_CALL, callId, procedure, params];
    client.send(JSON.stringify(msg));
    client.calls[callId] = callback;
  };

  client.callResult = function(callId, result){
    if (typeof this.calls[callId] === 'function') {
      client.calls[callId](result);
    }
  };
