...
handlers[protocol.TYPE_ID_CALL] = function(client, args) {
  ...
  this.emit('call', procUri, args, cb, client);
};
...
handlers[protocol.TYPE_ID_REVERSE_CALL_RESULT] = function(client, args) {
  callId = args.shift();
 
  this.clients[client.id].callResult(callId, args.shift());
  debug('client2server RPC call result for client ' + client.id + ' and callId ' + callId);
};
